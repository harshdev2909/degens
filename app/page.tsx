...
export default function LandingPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const dismissed = sessionStorage.getItem('instructionModalDismissed');
      if (!dismissed) setShowInstructionModal(true);
    }
  }, []);
  const handleCloseInstructionModal = () => {
    setShowInstructionModal(false);
    if (typeof window !== 'undefined') {
      sessionStorage.setItem('instructionModalDismissed', 'true');
    }
  };
  ...
  return (
    <>
      ...
      {showInstructionModal && (
        <Dialog open={showInstructionModal} onOpenChange={handleCloseInstructionModal}>
          <DialogContent className="max-w-md mx-auto">
            <DialogHeader>
              <DialogTitle className="text-2xl mb-2">How to Play</DialogTitle>
            </DialogHeader>
            <ol className="list-decimal pl-6 space-y-2 text-base text-gray-200">
              <li>Open <span className="font-bold text-purple-300">Backpack</span></li>
              <li>Create new <span className="font-bold text-blue-300">Solana wallet</span></li>
              <li>Set RPC → <span className="font-mono text-yellow-300">http://rpc.gorbagana.wtf</span></li>
              <li>Faucet → <span className="font-mono text-green-300">http://faucet.gorbagana.wtf</span></li>
              <li>Enter the arena → <span className="font-bold text-pink-300">Place your bet!</span></li>
            </ol>
            <div className="flex justify-end mt-4">
              <Button onClick={handleCloseInstructionModal} className="bg-purple-600 hover:bg-purple-700 text-white">Got it!</Button>
            </div>
          </DialogContent>
        </Dialog>
      )}
